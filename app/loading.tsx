...
import { FiHome, FiPackage, FiPlusCircle, FiSettings } from "react-icons/fi";
...
function ContentLoadingSkeleton({ hasSidebar = true }: { hasSidebar?: boolean }) {
  return (
    <main className={hasSidebar ? "ml-64 p-8" : "p-8"}>
      {/* Page title section */}
      <section className="mb-8">
        <LoadingPlaceholder customClass="h-8 w-32 mb-2" />
        <LoadingPlaceholder customClass="h-4 w-64" />
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <article className="bg-white rounded-lg border border-gray-200 p-6">
          <LoadingPlaceholder customClass="h-6 w-24 mb-6" />
          <div className="grid grid-cols-3 gap-6">
            {Array.from({ length: 3 }).map((_, idx) => (
              <div key={idx} className="text-center">
                <LoadingPlaceholder customClass="h-8 w-16 mx-auto mb-2" />
                <LoadingPlaceholder customClass="h-4 w-20 mx-auto mb-1" />
                <div className="flex items-center justify-center">
                  <LoadingPlaceholder customClass="h-3 w-8" />
                  <LoadingPlaceholder customClass="h-3 w-3 ml-1 rounded-full" />
                </div>
              </div>
            ))}
          </div>
        ...
        <article className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <LoadingPlaceholder customClass="h-6 w-40" />
          </div>
          <LoadingPlaceholder customClass="h-48 w-full" />
        </article>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <article className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <LoadingPlaceholder customClass="h-6 w-24" />
          </div>
            ...
            {Array.from({ length: 5 }).map((_, idx) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <LoadingPlaceholder customClass="h-4 w-24" />
                </div>
                <LoadingPlaceholder customClass="h-4 w-16" />
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <LoadingPlaceholder customClass="h-6 w-20" />
          </div>
          <div className="flex items-center justify-center">
            <LoadingPlaceholder customClass="w-48 h-48 rounded-full" />
          </div>
